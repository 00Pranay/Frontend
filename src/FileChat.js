...
export default function Dashboard() {
  ...
  const fetchFiles = async () => {
    try {
      const res = await fetch(
        `${process.env.REACT_APP_API_URL}/api/files/list?username=${username}`,
        { credentials: "include" }
      );
      if (!res.ok) return;
  ...
  const downloadFile = (filename) => {
    window.open(
      `${process.env.REACT_APP_API_URL}/api/files/download/${username}/${filename}`,
      "_blank"
    );
  };
  ...
  const deleteFile = async (filename) => {
    ...
    try {
      const res = await fetch("${process.env.REACT_APP_API_URL}/api/files/delete", {
        method: "POST",
  ...
  const shareSelectedFiles = async () => {
    ...
    for (const f of selectedFiles) {
      try {
        const res = await fetch("${process.env.REACT_APP_API_URL}/api/files/share", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const getFilePreview = (filename) => {
    ...
    const previewUrl = `${process.env.REACT_APP_API_URL}/api/files/preview/${username}/${filename}`;
